-- Store product catalog images in Supabase Storage instead of accepting
-- arbitrary external URLs from the admin and seller product forms.
...
CREATE UNIQUE INDEX product_images_storage_path_uidx
...
-- Replacing the rows through one RPC keeps the image metadata change atomic.
-- It is SECURITY INVOKER, so the existing product_images RLS policies remain
-- the source of admin/seller authorization.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN row_number() OVER (ORDER BY ordinal) = 1
        THEN true
      ELSE false
    END
  ...
  -- Respect the requested main image while still guaranteeing exactly one
  -- primary row whenever the product has images.
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(COALESCE(p_images, '[]'::jsonb)) WITH ORDINALITY AS source(image, ordinal)
