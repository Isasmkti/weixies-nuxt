...
<template>
  <div class="mx-auto max-w-6xl py-6 font-poppins">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-[0.18em] text-primary">Seller workspace</p><h1 class="mt-2 text-3xl font-black text-text-main">Payouts</h1><p class="mt-2 max-w-3xl text-text-muted">Earnings are protected for three days after payment, then sent automatically to your registered bank account. This page is read-only.</p></div><NuxtLink to="/seller" class="rounded-xl border border-bg-alt bg-surface px-4 py-2.5 text-sm font-bold text-text-main hover:text-primary">Back to dashboard</NuxtLink></div>
    ...
    <div v-else-if="payouts.length === 0" class="rounded-2xl border border-dashed border-bg-alt bg-surface p-10 text-center"><h2 class="text-xl font-bold text-text-main">No payouts yet</h2><p class="mt-2 text-sm text-text-muted">Automatic payout batches will appear here after an earning completes its three-day protection period.</p></div>
