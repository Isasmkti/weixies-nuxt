...
<template>
  <div class="mx-auto max-w-6xl py-6 font-poppins">
    ...
    <div v-else class="overflow-x-auto rounded-2xl border border-bg-alt bg-surface"><table class="w-full min-w-[820px] text-left"><thead class="bg-bg-alt/50 text-xs uppercase text-text-muted"><tr><th class="p-5">Period</th><th class="p-5">Amount</th><th class="p-5">Items</th><th class="p-5">Destination</th><th class="p-5">Reference</th><th class="p-5">Status</th></tr></thead><tbody class="divide-y divide-bg-alt"><tr v-for="payout in payouts" :key="payout.id"><td class="p-5 text-sm text-text-muted">{{ formatDate(payout.period_start) }} – {{ formatDate(payout.period_end) }}</td><td class="p-5 font-black text-text-main">{{ formatIDR(payout.amount) }}</td><td class="p-5 text-sm text-text-muted">{{ payout.seller_payout_items?.length || 0 }}</td><td class="p-5 text-sm text-text-main">{{ sellerOf(payout)?.bank_name || 'Not provided' }}<p class="font-mono text-xs text-text-muted">{{ sellerOf(payout)?.bank_account || '-' }}</p></td><td class="p-5 font-mono text-xs text-text-muted">{{ payout.reference_no || '-' }}</td><td class="p-5"><span class="rounded-full px-2.5 py-1 text-xs font-bold capitalize" :class="statusClass(payout.status)">{{ payout.status }}</span><p v-if="payout.paid_at" class="mt-2 text-xs text-text-muted">{{ formatDate(payout.paid_at) }}</p></td></tr></tbody></table></div>
  </div>
</template>
