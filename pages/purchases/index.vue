...
<template>
  <div class="mx-auto min-w-0 max-w-[1440px] py-4 md:py-6">
    ...
    <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <label class="relative block w-full sm:max-w-sm">
        ...
        <input v-model="search" type="search" maxlength="120" placeholder="Search purchases..." class="w-full rounded-ui-md border border-border bg-surface py-3 pl-10 pr-4 text-sm text-text-main outline-none transition placeholder:text-text-muted focus:border-primary focus:ring-2 focus:ring-primary/15">
      </label>
    ...
    <div v-if="purchases.loading" class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3" role="status" aria-label="Loading purchases">
      <div v-for="index in 6" :key="index" class="h-96 animate-pulse rounded-ui-lg border border-border bg-surface motion-reduce:animate-none"></div>
    </div>
    ...
    <section v-else class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-label="Purchased products">
      <article v-for="purchase in purchases.items" :key="purchase.id" class="flex min-w-0 flex-col overflow-hidden rounded-ui-lg border border-border bg-surface shadow-elevation-1">
        ...
        <div class="flex flex-1 flex-col p-4 sm:p-5">
          ...
          <details v-if="purchase.license" class="mt-4 rounded-ui-sm border border-border bg-bg/60 p-3 text-sm">
            ...
            <p class="mt-2 whitespace-pre-line text-xs leading-5 text-text-muted">{{ purchase.license.usage_terms }}</p>
          </details>
          ...
          <div class="mt-4 rounded-ui-sm bg-bg-alt/60 p-3">
            ...
            <p v-if="!purchase.file_available" class="mt-2 text-xs leading-5 text-text-muted">This file is temporarily unavailable. Contact the seller from your order details.</p>
          </div>

          <div class="mt-auto pt-4">
            <button type="button" :disabled="!purchase.can_download || downloading[purchase.product_id]" class="flex w-full items-center justify-center gap-2 rounded-ui-md bg-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-50" @click="download(purchase)">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-primary">
              <NuxtLink :to="`/orders/${purchase.order_id}`" class="hover:underline">Order details &amp; review</NuxtLink>
