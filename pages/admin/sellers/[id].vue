  ...
  <div class="mx-auto max-w-6xl font-poppins">
    ...
    <template v-else-if="seller">
      ...
      <section class="mt-6 grid gap-5 lg:grid-cols-3">
        <article class="rounded-2xl border border-bg-alt bg-surface p-5 lg:col-span-2"><h2 class="text-lg font-black text-text-main">Private payout information</h2><div class="mt-4 grid gap-4 sm:grid-cols-2"><div><p class="text-xs font-bold uppercase text-text-muted">Account holder</p><p class="mt-1 font-semibold text-text-main">{{ seller.payout_account_holder_name || 'Not provided' }}</p></div><div><p class="text-xs font-bold uppercase text-text-muted">Bank</p><p class="mt-1 font-semibold text-text-main">{{ seller.bank_name || 'Not provided' }}</p></div><div><p class="text-xs font-bold uppercase text-text-muted">Account number</p><p class="mt-1 font-mono font-semibold text-text-main">{{ seller.bank_account || 'Not provided' }}</p></div><div><p class="text-xs font-bold uppercase text-text-muted">Routing</p><p class="mt-1 text-sm font-semibold text-text-muted">Managed automatically by the platform</p></div><div><p class="text-xs font-bold uppercase text-text-muted">Profile ID</p><p class="mt-1 break-all font-mono text-sm text-text-main">{{ seller.profile_id }}</p></div><div><p class="text-xs font-bold uppercase text-text-muted">Joined</p><p class="mt-1 font-semibold text-text-main">{{ formatDate(seller.created_at) }}</p></div></div></article>
