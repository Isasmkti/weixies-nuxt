...
import { showErrorDialog, showSuccess, showToast } from '../../utils/sweetAlert'
...
const loadMonitor = async ({ notify = false } = {}) => {
  loading.value = true
  errorMessage.value = ''
  try {
    ...
    if (notify) showToast('Payout monitor updated')
  } catch (error) {
    errorMessage.value = error.message || 'Payout monitoring data could not be loaded.'
    if (notify) await showErrorDialog('Unable to refresh payouts', errorMessage.value)
  } finally {
    loading.value = false
  }
}

const canSyncPayout = payout => payout.provider === 'xendit'
  && payout.provider_payout_id
  && ['pending', 'processing', 'failed'].includes(payout.status)

const syncPayout = async (payout) => {
  if (!canSyncPayout(payout) || syncingId.value) return
  syncingId.value = payout.id
  try {
    await authFetch(`/api/admin/payouts/${payout.id}/sync`, { method: 'POST' })
    await loadMonitor()
    await showSuccess('Payout status synchronized', 'The latest status from Xendit is now shown in the payout history.')
  } catch (error) {
    await showErrorDialog('Payout could not be synchronized', error?.data?.statusMessage || error.message || 'The latest Xendit status could not be loaded.')
  } finally {
...
<template>
  <div class="mx-auto max-w-[1600px] font-poppins">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <button class="w-full rounded-ui-sm border border-border bg-surface px-4 py-2.5 text-sm font-semibold text-text-main transition hover:border-primary/40 hover:text-primary disabled:opacity-50 sm:w-auto" :disabled="loading" @click="loadMonitor({ notify: true })">{{ loading ? 'Refreshing...' : 'Refresh' }}</button>
    </div>
    ...
    <section class="mt-7 rounded-ui-lg border border-border bg-surface p-5 shadow-elevation-1 sm:p-6">
      ...
      <div v-else class="mt-5 overflow-hidden rounded-ui-md border border-border">
        <div class="divide-y divide-border md:hidden"><article v-for="candidate in candidates" :key="`mobile-${candidate.seller_id}`" class="space-y-3 p-4"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate font-bold text-text-main">{{ candidate.store_name }}</p><p class="mt-1 truncate font-mono text-xs text-text-muted">{{ candidate.bank_name || 'Bank missing' }} · {{ candidate.bank_account || '-' }}</p></div><span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold" :class="candidate.payout_ready ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'">{{ candidate.payout_ready ? 'Ready' : 'Incomplete' }}</span></div><div class="flex items-end justify-between gap-3"><p class="text-xs text-text-muted">{{ candidate.item_count }} items</p><p class="font-black text-text-main">{{ formatIDR(candidate.amount) }}</p></div></article></div>
    ...
    <section class="mt-7"><div class="mb-4"><h2 class="text-2xl font-black text-text-main">Payout history</h2><p class="mt-1 text-sm text-text-muted">Provider status is the source of truth; pending and processing rows are synchronized by the same daily job.</p></div>
      ...
      <div v-else class="overflow-hidden rounded-ui-lg border border-border bg-surface shadow-elevation-1">
        <div class="divide-y divide-border md:hidden">
          <article v-for="payout in payouts" :key="`mobile-${payout.id}`" class="space-y-4 p-4">
            <div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate font-bold text-text-main">{{ sellerOf(payout)?.store_name || 'Seller' }}</p><p class="mt-1 truncate text-xs text-text-muted">{{ payoutBank(payout).name || '-' }} · {{ payoutBank(payout).account || '-' }}</p></div><span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold capitalize" :class="statusClass(payout.status)">{{ payout.status }}</span></div>
            <dl class="grid grid-cols-2 gap-3 text-xs"><div class="rounded-ui-sm bg-bg p-3"><dt class="text-text-muted">Net amount</dt><dd class="mt-1 text-sm font-black text-text-main">{{ formatIDR(payout.amount) }}</dd></div><div class="rounded-ui-sm bg-bg p-3"><dt class="text-text-muted">Items</dt><dd class="mt-1 text-sm font-bold text-text-main">{{ payout.seller_payout_items?.length || 0 }}</dd></div><div class="col-span-2 rounded-ui-sm bg-bg p-3"><dt class="text-text-muted">Period</dt><dd class="mt-1 text-text-main">{{ formatDate(payout.period_start) }} – {{ formatPeriodEnd(payout.period_end) }}</dd></div></dl>
            <p class="break-all font-mono text-[10px] text-text-muted">{{ payout.reference_no || payout.provider_payout_id || '-' }}</p>
            <button v-if="canSyncPayout(payout)" type="button" class="min-h-10 w-full rounded-ui-sm border border-border px-3 py-2 text-xs font-bold text-text-main disabled:opacity-50" :disabled="Boolean(syncingId)" @click="syncPayout(payout)">{{ syncingId === payout.id ? 'Syncing...' : 'Sync Xendit' }}</button>
          </article>
        </div>
        <div class="hidden overflow-x-auto md:block">
        <table class="w-full min-w-[1160px] text-left">
          <thead class="bg-bg-alt/50 text-xs uppercase text-text-muted">
            <tr><th class="p-5">Seller</th><th class="p-5">Period</th><th class="p-5">Net amount</th><th class="p-5">Items</th><th class="p-5">Reference</th><th class="p-5">Status</th><th class="p-5 text-right">Action</th></tr>
          </thead>
          <tbody class="divide-y divide-border">
            <tr v-for="payout in payouts" :key="payout.id">
              <td class="p-5"><p class="font-bold text-text-main">{{ sellerOf(payout)?.store_name || 'Seller' }}</p><p class="text-xs text-text-muted">{{ payoutBank(payout).name || '-' }} · {{ payoutBank(payout).account || '-' }}</p></td>
              <td class="p-5 text-sm text-text-muted">{{ formatDate(payout.period_start) }} – {{ formatPeriodEnd(payout.period_end) }}</td>
              <td class="p-5"><p class="font-black text-text-main">{{ formatIDR(payout.amount) }}</p><p v-if="Number(payout.adjustment_amount)" class="mt-1 text-xs text-text-muted">Gross {{ formatIDR(payout.gross_amount) }} · adjustment {{ formatIDR(payout.adjustment_amount) }}</p></td>
              <td class="p-5 text-sm text-text-muted">{{ payout.seller_payout_items?.length || 0 }}</td>
              <td class="p-5"><p class="font-mono text-xs text-text-muted">{{ payout.reference_no || payout.provider_payout_id || '-' }}</p><p v-if="payout.provider_reference_id" class="mt-1 font-mono text-[10px] text-text-muted">{{ payout.provider_reference_id }}</p></td>
              <td class="p-5"><span class="rounded-full px-2.5 py-1 text-xs font-bold capitalize" :class="statusClass(payout.status)">{{ payout.status }}</span><p v-if="payout.provider_status" class="mt-2 text-xs font-semibold text-text-muted">Xendit: {{ payout.provider_status }}</p><p v-if="payout.provider_failure_code" class="mt-1 max-w-52 text-xs text-danger">{{ payout.provider_failure_code }}</p></td>
              <td class="p-5 text-right"><button v-if="canSyncPayout(payout)" type="button" class="rounded-ui-sm border border-border px-3 py-2 text-xs font-bold text-text-main transition hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-50" :disabled="Boolean(syncingId)" @click="syncPayout(payout)">{{ syncingId === payout.id ? 'Syncing...' : 'Sync Xendit' }}</button><span v-else class="text-xs text-text-muted">Up to date</span></td>
            </tr>
          </tbody>
        </table>
        </div>
