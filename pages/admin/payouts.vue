...
<template>
  <div class="mx-auto max-w-[1600px] font-poppins">
    <div class="mb-8"><h1 class="text-3xl font-extrabold tracking-tight text-text-main sm:text-4xl">Seller Payouts</h1><p class="mt-2 text-text-muted">Create payout batches from paid, unreleased seller order items.</p></div>
    ...
    <section class="rounded-2xl border border-bg-alt bg-surface p-5 shadow-sm sm:p-6">
      <h2 class="text-xl font-black text-text-main">Create payout batches</h2><p class="mt-1 text-sm text-text-muted">The end date is inclusive. Existing active payout items are excluded automatically.</p>
    ...
    <section class="mt-7"><div class="mb-4 flex items-center justify-between"><h2 class="text-2xl font-black text-text-main">Payout history</h2><button class="text-sm font-bold text-primary hover:underline" :disabled="loadingPayouts" @click="loadPayouts">Refresh</button></div>
      ...
      <div v-else class="overflow-x-auto rounded-2xl border border-bg-alt bg-surface"><table class="w-full min-w-[1050px] text-left"><thead class="bg-bg-alt/50 text-xs uppercase text-text-muted"><tr><th class="p-5">Seller</th><th class="p-5">Period</th><th class="p-5">Amount</th><th class="p-5">Items</th><th class="p-5">Reference</th><th class="p-5">Status</th><th class="p-5 text-right">Actions</th></tr></thead><tbody class="divide-y divide-bg-alt"><tr v-for="payout in payouts" :key="payout.id"><td class="p-5"><p class="font-bold text-text-main">{{ sellerOf(payout)?.store_name || 'Seller' }}</p><p class="text-xs text-text-muted">{{ sellerOf(payout)?.bank_name || '-' }} · {{ sellerOf(payout)?.bank_account || '-' }}</p></td><td class="p-5 text-sm text-text-muted">{{ formatDate(payout.period_start) }} – {{ formatDate(payout.period_end) }}</td><td class="p-5 font-black text-text-main">{{ formatIDR(payout.amount) }}</td><td class="p-5 text-sm text-text-muted">{{ payout.seller_payout_items?.length || 0 }}</td><td class="p-5 font-mono text-xs text-text-muted">{{ payout.reference_no || '-' }}</td><td class="p-5"><span class="rounded-full px-2.5 py-1 text-xs font-bold capitalize" :class="statusClass(payout.status)">{{ payout.status }}</span></td><td class="p-5"><div v-if="!['paid', 'failed'].includes(payout.status)" class="flex justify-end gap-2"><button v-if="payout.status === 'pending'" class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-60" :disabled="workingId === payout.id" @click="updatePayout(payout, 'processing')">Processing</button><button class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-60" :disabled="workingId === payout.id" @click="updatePayout(payout, 'paid')">Mark paid</button><button class="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-60" :disabled="workingId === payout.id" @click="updatePayout(payout, 'failed')">Failed</button></div><span v-else class="block text-right text-xs text-text-muted">Completed</span></td></tr></tbody></table></div>
    </section>
  </div>
</template>
