  ...
  <div class="mx-auto max-w-[1120px] font-[Inter,sans-serif]">
    ...
    <div v-else-if="product" class="flex flex-col gap-8">
      ...
      <section class="product-overview">
        ...
        <aside class="product-buy-panel flex min-w-0 flex-col gap-4 rounded-2xl border border-bg-alt bg-surface p-5 shadow-lg shadow-black/[0.03] md:sticky md:top-6 md:p-6">
      ...
      <section class="rounded-2xl border border-bg-alt bg-surface p-6 shadow-sm md:p-8">
        <div class="flex items-center gap-3"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5A3.375 3.375 0 0 0 10.125 2.25H8.25M8.25 15h7.5M8.25 18H12m-5.25 3.75h10.5a2.25 2.25 0 0 0 2.25-2.25V11.25a9 9 0 0 0-9-9h-3.75A2.25 2.25 0 0 0 4.5 4.5v15a2.25 2.25 0 0 0 2.25 2.25Z" /></svg></span><div><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-primary">About this product</p><h2 class="mt-0.5 text-2xl font-black text-text-main">Product description</h2></div></div>
        <div class="mt-6 max-w-4xl whitespace-pre-line text-sm leading-7 text-text-muted md:text-base md:leading-8">{{ product.description || 'No description is available for this product.' }}</div>
      </section>

      <section v-if="productSpecs.length" class="rounded-2xl border border-bg-alt bg-surface p-6 shadow-sm md:p-8">
        <div class="flex items-center justify-between gap-4"><div><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-primary">Technical information</p><h2 class="mt-1 text-2xl font-black text-text-main">Product specifications</h2></div><span class="rounded-full bg-bg-alt px-3 py-1 text-xs font-bold text-text-muted">{{ productSpecs.length }} details</span></div>
        <dl class="mt-6 overflow-hidden rounded-xl border border-bg-alt">
          <div v-for="(spec, index) in productSpecs" :key="spec.id" class="grid gap-1 px-5 py-4 sm:grid-cols-[minmax(10rem,0.8fr)_minmax(0,1.2fr)] sm:gap-6" :class="index !== productSpecs.length - 1 ? 'border-b border-bg-alt' : ''">
